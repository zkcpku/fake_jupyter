...
def to_dict(node,add_list):
    if type(node) == type(" "):
        add_list.append({'name':node,'children':[],'collapsed': False})
        return
    this_dict = {'name':node.production.constructor.name,'children':[],'collapsed': False}
...
def get_ast_dict(py_code):
    ...
    return {'name':'module','children':rsts,'collapsed': False}
